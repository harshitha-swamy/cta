...
const sample = `
<svg xmlns="http://www.w3.org/2000/svg" width="249" height="64" viewBox="0 0 249 64">
  <rect width="249" height="64" fill="#ffffff" stroke="#0f172a" stroke-width="2" data-background="true"/>
  <text x="50%" y="50%" text-anchor="middle" dominant-baseline="middle" font-size="14" fill="#0f172a">Sample</text>
</svg>
`;
...
async function html_translateText(original, targetLang) {
  ...
  if (targetLang !== 'en' && html_translations[targetLang] && html_translations[targetLang][key]) {
    return html_translations[targetLang][key];
  }
  if (targetLang === 'fr-CA') {
    try {
      const resp = await fetch(`https://api.mymemory.translated.net/get?q=${encodeURIComponent(original)}&langpair=en|fr`);
      const data = await resp.json();
      return data.responseData.translatedText || original;
    } catch (err) {
      console.error('Translation API error:', err);
    }
  }
